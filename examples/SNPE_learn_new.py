...
def lo_simulation(rates):
    ...
    sbml_model = Model(sbml_filename=r"/home/kohler.d/biosim_project/sbi/examples/LacOperon_stochastic.xml",
                       sbml_warnings=False)
...
def main():
    ...
    for i in range(len(parameters)):
        ...
        with open(r'/scratch/kohler.d/code_output/biosim/SNPE_learn_new_obs_{0}.pickle'.format(str(i)), 'wb') as handle:
...
#
# plot_df = [prior_obs[i].reshape((int(len(prior_obs[0])/12),12)) for i in range(len(prior_obs))]
# plot_df1 = [new_obs[i].reshape((int(len(new_obs[0])/12),12)) for i in range(len(new_obs))]
#
# fig, ax = plt.subplots(2,1, figsize=(12,8))
#
# n = 0
# for i in [plot_df, plot_df1]:
