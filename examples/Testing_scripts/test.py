# UserWarning: An x with a batch size of 3 was passed. It will be interpreted as a batch of independent and identically
...
prior = utils.NormalPrior(
    torch.tensor([0.01, 0.0005, 0.003]),
    torch.tensor([0.001, 0.000005, 0.0001])
)
...
obs = torch.tensor([gillespie_simulator([0.015, 0.0005, 0.003]).numpy() for _ in range(200)])

plot_df = np.array([i for i in obs[0] if i > 0.])
plot_df = plot_df.reshape((int(len(plot_df)/3),3))
plot_df1 = np.array([i for i in obs[1] if i > 0.])
plot_df1 = plot_df1.reshape((int(len(plot_df1)/3),3))
plot_df2 = np.array([i for i in obs[2] if i > 0.])
